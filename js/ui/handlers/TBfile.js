...
function showReceiverNotification(peerId, requestId, fromName) {
  const n = document.createElement("div");
  n.className = "tbfile-topnotif";
  n.innerHTML = `
    <div><strong>${fromName}</strong> would like to share a file with you</div>
    <button class="tb-accept">Accept</button>
    <button class="tb-deny">Deny</button>
  `;
  document.body.appendChild(n);

  n.querySelector(".tb-accept").onclick = () => {
    TBPeerManager.send(peerId, {
      type: "file-response",
      id: requestId,
      accepted: true,
  ...
  n.querySelector(".tb-deny").onclick = () => {
    TBPeerManager.send(peerId, {
      type: "file-response",
      id: requestId,
      accepted: false,
...
export function initTBfile() {
  ...
  TBPeerManager.onRequest = (peerId, requestId, fromName) => {
    showReceiverNotification(peerId, requestId, fromName);
  };
